...
# 4 comp medium
X <- df.med %>% filter(comp%in%c(1,2,4,5)) %>% select(V1,V2) %>% cbind(rep(1,
nrow(filter(df.med,comp%in%c(1,2,4,5)))), .)
y <- df.med %>% filter(comp%in%c(1,2,4,5)) %>% select(y) 

m4m <- stan(model_code = model_string, data = list(X=as.matrix(X), M=ncol(X),
...
save.image(file="/home/igm/christoph.kurz/R/dpmix/simstudyc4m.Rdata")
