...
# 3 comp high
X <- df.hvy %>% filter(comp%in%c(1,2,5)) %>% select(V1,V2) %>% cbind(rep(1,
nrow(filter(df.hvy,comp%in%c(1,2,5)))), .)
y <- df.hvy %>% filter(comp%in%c(1,2,5)) %>% select(y) 

m3h <- stan(model_code = model_string, data = list(X=as.matrix(X), M=ncol(X),
...
save.image(file="/home/igm/christoph.kurz/R/dpmix/simstudyc3h.Rdata")
