...
# 4 comp low
X <- df.low %>% filter(comp%in%c(1,2,4,5)) %>% select(V1,V2) %>% cbind(rep(1,
nrow(filter(df.low,comp%in%c(1,2,4,5)))), .)
y <- df.low %>% filter(comp%in%c(1,2,4,5)) %>% select(y) 

m4l <- stan(model_code = model_string, data = list(X=as.matrix(X), M=ncol(X),
...
save.image(file="/home/igm/christoph.kurz/R/dpmix/simstudyc4l.Rdata")
